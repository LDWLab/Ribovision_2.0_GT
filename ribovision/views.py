...
def fetchresidues(request):
    if request.method == "POST":
        ...
        SQLStatement = 'SELECT ss.map_Index, ss.molName, resNum, X, Y, unModResName, modResName, MoleculeType, \
                    ...
                    FROM (SELECT * FROM SecondaryStructures WHERE SS_Table = '+structure_identity+') AS ss \
                    INNER JOIN MoleculeNames AS mn ON ss.molName = mn.MoleculeName \
                    INNER JOIN (SELECT MoleculeName, ChainName FROM ChainList \
                    WHERE StructureName = (SELECT DISTINCT StructureName FROM Secondary_Tertiary WHERE SS_Table = '+structure_identity+')) \
                    AS cl ON cl.MoleculeName = ss.molName \
                    LEFT JOIN (SELECT * FROM StructuralData2 WHERE SS_Table = '+structure_identity+') AS sd ON sd.map_Index = ss.map_Index'
