...
export class UiActionsService {
    ...
    static colorPath(pdbId: string, label_seq_id: number, color?: string, type?: 'selection' | 'highlight') {
        ...
        (<any>document.querySelector(`svg.rnaTopoSvg.rnaTopoSvg_${pdbId}`))!.getElementsByClassName(`rnaviewEle rnaviewEle_${pdbId} rnaview_${pdbId}_${label_seq_id}`)[0].setAttribute("fill",strokeColor);
    }

    static clearHighlight(pdbId: string) {
        if (this.selected > -1) {
            (<any>document.querySelector(`svg.rnaTopoSvg.rnaTopoSvg_${pdbId}`))!.getElementsByClassName(`rnaviewEle rnaviewEle_${pdbId} rnaview_${pdbId}_${this.selected}`)[0].setAttribute("fill","323232");
            this.selected = -1;
        }
