<template>
    <div id="phylo_tree_dropdown">
        <div class="left-sidebar">
            <div id="tree_type" class="btn-group btn-group-toggle" data-toggle="buttons">
                ...
                <label class="btn btn-outline-dark" style="margin: 0 1% 0 0;width:100%; z-index: 0;" for="orthologs">
                    <input type="radio" id="orthologs" value="orth" v-model="type_tree" v-on:input="cleanTreeOpts()"
                        checked>
                    RiboVision
                </label>

                <label class="btn btn-outline-dark" style="margin: 0 0 0 1%;width:50%; z-index: 0;" for="upload">
            ...
            <div id="treeselect" v-if="type_tree == 'para' | type_tree == 'orth'">
                <treeselect ref="treeselect" :load-options="loadOptions" v-model="tax_id"
                    v-on:input="loadProteinTypes(tax_id, type_tree)" placeholder="Select a phylogenetic group"
                    no-children-text="Loading... or no children" :multiple="true" :options="options" :flat="true"
                    :limit="3" :default-expand-level="1">Loading phylogenetic tree <img src="static/img/loading.gif"
                        alt="Loading phylogenetic tree" style="height:25px;">
                </treeselect>
            </div>
            <div v-else>
                Select an alignment file:
                <p><input id="inputUploadFasta" class="btn btn-outline-dark" type="file" accept=".fasta,.fas,.fa"
                        ref="custom_aln_file" v-on:change="handleFileUpload()" /></p>
                <p><button id="uploadShowFasta" class="btn btn-outline-dark" v-on:click="submitCustomAlignment()"
                        v-if="file && type_tree == 'upload'">Upload the alignment</button></p>
                <p><button id="downloadExampleFasta" class="btn btn-outline-dark"
                        v-on:click="getExampleFile(`static/alignments/EFTU_example.fas`, `PVExampleAlignment.fas`)"
                        v-if="!file && type_tree == 'upload'">Download example alignment</button></p>
            </div>
            <p>
                <select class="btn btn-outline-dark dropdown-toggle" id="select_protein_type" v-if="tax_id"
                    v-model="protein_type_obj" v-on:change="loadData(tax_id, type_tree)"
                    style="margin: 0 1% 0 0;width:100%;">
            ...
            <p>
                <select class="btn btn-outline-dark dropdown-toggle" id="selectaln" v-if="protein_type_obj && tax_id"
                    v-model="alnobj" v-on:change="handleAlignChange()">
                    <option :value="null" selected disabled hidden>Select an alignment</option>
                    <option v-for="aln in alignments" v-bind:key=aln.txt
                        v-bind:value="{ id: aln.value, text: aln.text }">{{ aln.text }}</option>
                </select>
            </p>
            <div v-if="alnobj && alnobj == 'custom' && file && type_tree == 'upload'">
                ...
                <div v-if="cifPdbMode == 'CIF'">
                    ...
                    Upload a custom CIF:
                    <br />
                    <label for="uploadCustomCIF" id="cif-upload" class="btn btn-outline-dark"
                        style="margin: 0 1% 0 0;width:100%;">Choose File</label>
                    <input id="uploadCustomCIF" class="btn btn-outline-dark" type="file" accept=".cif"
                        ref="customCIFfile" v-on:change="cifFileUploadedFlag = true;"
                        style="margin: 0 1% 0 0;width:100%;" />
                    <div v-if="cifFileUploadedFlag">
                        <label for="provideEntityID" style="margin: 0 1% 0 0;width:100%;">Provide an Entity ID for a
                            desired RNA chain</label>
                        <input id="provideEntityID" class="btn btn-outline-dark" type="number" ref="entity_id"
                            v-on:change="uploadCustomCIF();" placeholder="Entity ID"
                            style="margin: 0 1% 0 0;width:100%;" />
                    </div>
                </div>
                <div v-if="cifPdbMode == 'PDB'">
                    ...
                    <input id="uploadCustomPDB" class="btn btn-outline-dark" type="file" accept=".pdb"
                        ref="customPDBfile" v-on:change="pdbFileUploadedFlag = true;" />
                    <div v-if="pdbFileUploadedFlag">
                        Upload the matching RNA sequence:
                        <br />
                        <label for="uploadCustomFullSequence" id="full-sequence-upload" class="btn btn-outline-dark"
                            style="margin: 0 1% 0 0;width:100%;">Choose File</label>
                        <input id="uploadCustomFullSequence" class="btn btn-outline-dark" type="file"
                            accept=".fas,.fasta,.fa" ref="customFullSequence" v-on:change="uploadCustomFullSequence();"
                            style="margin: 0 1% 0 0;width:100%;" />
                    </div>
                </div>
            ...
            <p>
                ...
                <autocomplete id="pdb_input" isAsync:true :items="pdbs" v-if="alnobj && alnobj != 'custom'" v-model="pdbid">
                </autocomplete>
            </p>
            <p><select multiple class="form-control btn-outline-dark" id="polymerSelect"
                    v-bind:style="{ resize: 'both' }" v-if="alnobj && chains && fasta_data && pdbid || uploadSession"
                    v-model="chainid">
                    <option :value="null" selected disabled>Matching RNA chain to visualize:</option>
                    <option v-for="chain in chains" v-bind:key="chain.value" v-bind:value="chain.value"
                        @click="selectedProteins = []; postStructureData(pdbid, chainid); calculateProteinContacts(pdbid, chainid); populateECODranges(pdbid, chainid); showPDBViewer(pdbid, chainid, guideOff ? chain.entityID : RVGuideEntityId);">
                        {{ chain.text }}</option>
                    </select></p>
            
            <div v-if="structure_mapping && chains && !pdbcust && !cifcust">
                <select id="downloadDataBtn" class="btn btn-outline-dark dropdown-toggle" v-model="downloadMapDataOpt"
                    v-if="topology_loaded">
            ...
            <p>
            <div v-if="topology_loaded && type_tree == 'orth'" class="checkbox" id="showRNAcontext">
            ...
            <div v-if="modified && cifcust && structure_mapping">
                <form @submit.prevent="submitModificationsCustom">
                    ...
                    <div id="modSelectCustom" class="selection-box"
                        :style="{ backgroundColor: 'white', padding: '10px', border: '1px solid #ddd', marginBottom: '10px', maxHeight: '100px', overflowY: 'auto' }">

                        <div>
                            <input type="checkbox" id="selectAllModifiedCustom" v-model="selectAllModifiedCustomChecked"
                        ...
                        <div v-for="[text, k] of modified_residues.entries()" :key="k">
                            <input type="checkbox" :id="text" :value="text" v-model="selectedResiduesCustom" />
            ...
            <div v-if="topology_loaded && !checkedRNA">
                ...
                <div id="customDataSection">
                    <p>
                    <div class="checkbox">
                        <label><input type="checkbox" id="uploadCustomData" v-model="checked_customMap"
                                v-on:change="cleanCustomMap(checked_customMap)">
                            Upload custom mapping data</label>
                        <p><input class="btn btn-outline-dark" id="inputUploadCSV" v-if="checked_customMap" type="file"
                                accept=".csv" ref="custom_csv_file" v-on:change="handleCustomMappingData()" /></p>
                        <p v-if="raiseCustomCSVWarn" v-html="raiseCustomCSVWarn"></p>
                        <p><button class="btn btn-outline-dark" id="downloadExampleCSV"
                                v-if="checked_customMap && csv_data == null" type="button"
                                v-on:click="getExampleFile(`static/alignments/rv3_example_cusom_mapping.csv`, `RVExampleCustomMapping.csv`)">
                ...
                <div v-if="topology_loaded && protein_contacts">
                    <form @submit.prevent="submitProteins">
                        ...
                        <div id="proteinSelect" class="selection-box"
                            :style="{ backgroundColor: 'white', padding: '10px', border: '1px solid #ddd', marginBottom: '10px', maxHeight: '100px', overflowY: 'auto' }">
                            <div>
                                <input type="checkbox" id="selectAllProteins" v-model="selectAllProteinsChecked"
                            ...
                            <div v-for="chain in protein_chains" :key="chain.key">
                                <input type="checkbox" :id="chain.value" :value="chain.value"
                ...
                <div v-if="modified && !cifcust">
                    <form @submit.prevent="submitModifications">
                        <label>Select modified residues to highlight</label>
                        <div id="modSelect" class="selection-box"
                            :style="{ backgroundColor: 'white', padding: '10px', border: '1px solid #ddd', marginBottom: '10px', maxHeight: '100px', overflowY: 'auto' }">

                            <div>
                                <input type="checkbox" id="selectAllModified" v-model="selectAllModifiedChecked"
                                    @change="selectAllModifiedChanged" />
                                <label for="selectAllModified">Select All</label>
                            </div>
                            <div v-for="[text, k] of modified_residues.entries()" :key="k">
                                <input type="checkbox" :id="text" :value="text" v-model="selectedResidues" />
                                <label :for="text">{{ text }}</label>
                            </div>
        ...
        <div class="alignment_section">
            <div id="alnif" v-if="alnobj">
                <div id="alnMenu" style="display: flex;">
                    <button id="downloadFastaBtn" class="btn btn-outline-dark" style="margin: 0 1%;"
                        v-if="msavWillMount" type="button" v-on:click="downloadAlignmentData()">
                        Download alignment
                    </button>
                    <select id="cdHITResults" class="btn btn-outline-dark dropdown-toggle" style="margin: 0 1%;"
                        v-model="cdhitSelectedOpt" v-if="cdHITReport">
                    ...
                    <select id="downloadAlnImageBtn" class="btn btn-outline-dark dropdown-toggle" style="margin: 0 1%;"
                        v-model="downloadAlignmentOpt" v-if="msavWillMount">
                    ...
                    <select id="selectColorMappingProps" class="btn btn-outline-dark dropdown-toggle"
                        style="margin: 0 1%;" v-model="selected_property" v-if="msavWillMount">
                    ...
                    <select id="selectAlnColorScheme" class="btn btn-outline-dark dropdown-toggle" style="margin: 0 1%;"
                        v-model="colorScheme" v-if="msavWillMount">
                ...
                <div id="alnDiv">Loading alignment <img src="static/img/loading.gif" alt="Loading alignment"
                        style="height:25px;"></div>
            </div>
        </div>
        <div class="warningSection">
            <div id="warningCDHITtruncation" v-if="cdHITReport && didCDHit_truncate">
                <b>Warning, your alignment sequences were clustered by cdhit! See dropdown menu above the alignment for
                    options.<br />
                    Original alignment had {{ this.cdHITnums[0] }} sequences, which were clustered in
                    {{ this.cdHITnums[1] }} groups using threshold of 90% identity.</b>
            </div>
        ...
        <div class="topology_section">
            <span id="topif" v-if="chainid.length > 0 || customPDBsuccess">
                <div v-if="!topology_loaded">
                    Wait for alignment-structure mapping <img src="static/img/loading.gif" alt="Loading topology viewer"
                        style="height:25px;">
                </div>
                <div id="topview"></div>
            </span>
        ...
        <div class="gradient_section" v-if="topology_loaded">
            <img id='gradientSVG' v-for="prop in available_properties" :key="prop.Name"
                v-if="selected_property == prop.Name" :src="prop.url">
        </div>
        <div class="molstar_section">
            <div v-if="PDBparsing == true">
                Parsing PDB structure <img src="static/img/loading.gif" alt="Parsing PDB structure"
                    style="height:25px;">
            </div>
            ...
            <span id="molif" v-if="chainid.length > 0 || customPDBsuccess">
                <div id="pdbeMolstarView">
                    Loading Molstar Component <img src="static/img/loading.gif" alt="Loading MolStar"
                        style="height:25px;">
                </div>
            </span>
        </div>
