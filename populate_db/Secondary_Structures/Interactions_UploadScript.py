...
def get_resi_j_fk(residue_j):
    cursor.execute("SELECT * FROM SEREB.Residues\
...
for entry in csv_list[1:]:
    ...
    _3D_structure_id = get_3D_structure_id(entry[1])
            #query = "INSERT INTO `SEREB`.`Interactions`(`interactions_id`,`residue_i`,`residue_j`,`bp_type`,`bp_group`,`3D_structure_id`) VALUES('"+entry[0]+"','"+str(resi_id)+"','"+str(resi_id)+"','"+(entry[3])+"','"+entry[4]+"','"+str(_3D_structure_id)+"')"
            #print(query)
