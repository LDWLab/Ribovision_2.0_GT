...
def execute_old_name_query(row, occurrence, lastrow_id):
	...
	if occurrence == 'B':
		if row[3]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[3]+"','"+occurrence+"')")
			cursor.execute(query)
	elif occurrence == 'A':
		if row[6]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[6]+"','"+occurrence+"')")
			cursor.execute(query)
	elif occurrence == 'E':
		if row[4]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[4]+"','Y')")
			cursor.execute(query)
		if row[5]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[5]+"','H')")
			cursor.execute(query)
	elif occurrence == 'm':
		if row[3]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[3]+"','B')")
			cursor.execute(query)
		if row[4]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[4]+"','mY')")
			cursor.execute(query)
		if row[5]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[5]+"','mH')")
			cursor.execute(query)
	elif occurrence == 'c':
		if row[3]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[3]+"','B')")
			cursor.execute(query)
		if row[4]:
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[4]+"','cY')")
			cursor.execute(query)
	else:
...
def upload_csv(csvList, cursor):
	...
	for row in csvList:
		for occurrence in list(row[2]):
			if row[0][1] == 'S':
				query = ("INSERT INTO `SEREB2`.`Nomenclature`(`new_name`,`occurrence`,`MoleculeGroup`) VALUES('"+row[0]+"','"+occurrence+"','SSU')")
			elif row[0][1] == 'L':
				query = ("INSERT INTO `SEREB2`.`Nomenclature`(`new_name`,`occurrence`,`MoleculeGroup`) VALUES('"+row[0]+"','"+occurrence+"','LSU')")
			else:
			...
			query = ("INSERT INTO `SEREB2`.`Old_name`(`nomo_id`,`old_name`,`N_B_Y_H_A`) VALUES('"+str(lastrow_id)+"','"+row[1]+"','BAN')")
			cursor.execute(query)
	cnx.commit()
	return True
