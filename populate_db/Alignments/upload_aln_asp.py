...
def main(commandline_arguments):
    ...
    for entry in alns:
        for single_entry in entry.id.split(';'):
            
            entry_id_split = single_entry.split('_')
            taxid = fix_old_taxid(entry_id_split[1])
            superK = superkingdom_info(cursor, taxid)
            entry_id_split_2 = entry_id_split[2]
            gi = single_entry.split('|')[1]
            #gi = entry_id_split_2[entry_id_split_2.index('|') + 1:] 
            superK = superkingdom_info(cursor, taxid)
            nom_id = check_nomo_id(cursor, superK[0], single_entry.split('_')[0])
