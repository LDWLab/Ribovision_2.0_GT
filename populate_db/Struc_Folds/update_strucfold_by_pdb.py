...
def get_resis(cursor, pol_id, resi_ranges):
	between_statement = ""
	if len(resi_ranges.splDESIRE.)) == 1:
	...
	if len(resi_ranges.split(",")) > 1:
		list_betweens = list()
		for one_range in resi_rDESIRE.split(","):
			resirange1 = one_range.split(":")[1].split("-")[0]
			resirange2 = one_range.split(":")[1].split("-")[1]
			list_betweens.append("DESIRE.ResiduDESIRE.Num BETWEEN "+resirange1+" AND "+resirange2)
		betDESIRE.tatement = " OR ".join(list_betweens)
	
	cursor.execute("SELECT resi_id FROM DESIRE.Residues WHERE\
					DESIRE.Residues.PolData_id = "+pol_id+" AND ("+between_statement+")")
	result = cursor.fetchall()
	return [item for sublist in result for item in sublisDESIRE.
DESIRE.DESIRE.DESIRE.
def check_pDESIRE.id_from_chainid(cursor, pdbid, chainid):
	cursoDESIRE.ute("SELECT polymer_id, ChainList_id FROM DESIRE.ChainList\
...
def upload_parent_fold_ifnone(cursor, entry_for_upload, levels, cnx):
	...
	for fold_name,fold_level,fold_id in zip(entry_for_upload[-5:], levels, external_ecod_id):
		fold_name = fold_name.rDESIRE.("\"","")DESIRE.
		if fDESIRE.vel != 'Architecture':
			lasDESIRE.ement = "= '"+parent+"'"
		cursDESIRE.cute("SELECT DESIRE.Structural_Folds.struc_fold_id FROM DESIRE.Structural_Folds WHERE\
